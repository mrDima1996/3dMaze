     ...
     return function() {
         ...
         if (keysPressed[32]) { //нажали пробел
             if (!isShot) {
                 shot();
                 isShot = true;
                 setTimeout(function() {isShot = false}, 100);
             }
         }
...
/**
 * функция передвигает переданную пулю
 * @param obj - three.js объект пули
 */
function bulletMove(obj){
    ...
    if (!isInMaze(obj)) view._scene.remove(obj);
}

/**
 * проверяет не вышел ли объект за пределы лабиринта
 * @param obj - three.js объект
 */
function isInMaze(obj) {
    var position = obj.position;
    if ((position.x<0) || (position.y<0) || (position.z>0) ) return false;
    if ((position.x>model.borderX) || (position.y>4) || (position.z<model.borderZ) ) return false;
    return true;
};
